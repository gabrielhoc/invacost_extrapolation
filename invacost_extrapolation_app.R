...
ui = fluidPage(
  tags$head(HTML("<title>Economic Cost of invasive species around the world</title>")),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectizeInput(inputId = "Sector",
                     label = "Choose Impacted Sector:",
                     choices = choices_sector,
                     options = list(
                       placeholder = 'Please select Impacted Sector',
      ...
      selectizeInput(inputId = "group",
                     ...
                     options = list(
                       placeholder = 'Please select Taxonomic Group',
                       onInitialize = I('function() { this.setValue(""); }')), 
                     multiple = TRUE),
...
server = function(input, output) {
  ...
  output$bar_plot <- renderPlot(bar_plot(), height = 300, width = 1200)
