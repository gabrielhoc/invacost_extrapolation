...
- Adjust RF hyperparameters
- add time since introduction
- use taxonomic groups in Seebens 2017?
...
inva_db_clean <- 
  inva_db[inva_db$Method_reliability == "High" & 
            ...
            !stringr::str_detect(inva_db$Species, "/") &
            !stringr::str_detect(inva_db$Official_country, "/") &
            !stringr::str_detect(inva_db$Impacted_sector, "/"),]
```
...
psce_df <-
  data.frame(
    ...
    Cost = inva_db_clean$Cost_estimate_per_year_2017_USD_PPP,
...
psce_unq <- psce_unq[!psce_unq$Phylum %in% c("Diverse", "Unspecified", "Diverse/Unspecified"),]
...
inds_df <- inds_df[complete.cases(inds_df),]
```
...
psce_boruta_preds <- 
  Boruta::Boruta(log_Cost ~ .,
                 psce_train_preds[,-c(1,4,5,8)])

selected_preds <-
  names(psce_boruta_preds$finalDecision)[psce_boruta_preds$finalDecision == "Confirmed"]

View(clean_indicators[clean_indicators$indicator_id %in% selected_preds,])
```

```{r}
psce_train_selected <- 
  psce_train_preds[c(7, which(names(psce_train_preds) %in% selected_preds))]
```
...
1 - mean((pred_psce_rf_preds - 
            psce_test_preds$log_Cost)^2)/
  var(psce_test_preds$log_Cost)
```
